...
data$wakeup<- format(strptime(data$Waking.up.time, format = "%H:%M"), "%H:%M")
View(data)
class(data$wakeup)
data$wakeup2<- as.numeric(data$wakeup)
data$wakeup2<- ifelse(data$wakeup > 07:30 | data$wakeup < 07:30, "Late", "Early") 
...
boxplot(Exercise ~ wakeup, data= data)
```

```{r}
lm1<- lm(Exercise~wakeup, data = data) #most basic lm
summary(lm1)
